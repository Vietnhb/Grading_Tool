        ...
        child: Column(
          children: [
            for (var index = 0; index < rows.length; index += 1)
              _GuideRow(row: rows[index], last: index == rows.length - 1),
          ],
        ),
      ),
    );
...
class _GuideRow extends StatelessWidget {
  const _GuideRow({required this.row, required this.last});
      ...
      child: Row(
        children: [
          for (var index = 0; index < row.cells.length; index += 1)
            Expanded(
              flex: index == row.cells.length - 1 ? 12 : 36,
              child: _GuideCell(
                text: row.cells[index],
                emphasized: row.isHeader,
                pointsColumn: index == row.cells.length - 1,
